...
-- NIP-50: Full-text search virtual table (external content, reads from event)
CREATE VIRTUAL TABLE IF NOT EXISTS event_fts USING fts5(content, content='event', content_rowid='id');
...
CREATE TRIGGER IF NOT EXISTS event_fts_delete AFTER DELETE ON event BEGIN
  INSERT INTO event_fts(event_fts, rowid, content) VALUES ('delete', old.id, old.content);
END;
CREATE TRIGGER IF NOT EXISTS event_fts_update AFTER UPDATE ON event BEGIN
  INSERT INTO event_fts(event_fts, rowid, content) VALUES ('delete', old.id, old.content);
  INSERT INTO event_fts(rowid, content) VALUES (new.id, new.content);
END;
...
fn mig_18_to_19(conn: &mut PooledConnection) -> Result<usize> {
    info!("database schema needs update from 18->19 (NIP-50 FTS5)");
    let upgrade_sql = r##"
-- NIP-50: Full-text search virtual table (external content, reads from event)
CREATE VIRTUAL TABLE IF NOT EXISTS event_fts USING fts5(content, content='event', content_rowid='id');

-- Triggers to keep event_fts in sync with event
CREATE TRIGGER IF NOT EXISTS event_fts_insert AFTER INSERT ON event BEGIN
  INSERT INTO event_fts(rowid, content) VALUES (new.id, new.content);
END;
CREATE TRIGGER IF NOT EXISTS event_fts_delete AFTER DELETE ON event BEGIN
...
-- Rebuild FTS index from existing event data
...
"##;
    match conn.execute_batch(upgrade_sql) {
        Ok(()) => {
            info!("database schema upgraded v18 -> v19 (NIP-50 FTS5)");
        }
        Err(err) => {
