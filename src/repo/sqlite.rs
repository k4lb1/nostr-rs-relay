...
impl NostrRepo for SqliteRepo {
    ...
    /// Admit account
    async fn admit_account(&self, pub_key: &str, admission_cost: u64) -> Result<()> {
        let mut conn = self.write_pool.get()?;
        let pub_key = pub_key.to_string();
        tokio::task::spawn_blocking(move || {
            let tx = conn.transaction()?;
    ...
    async fn get_unpaid_invoice(&self, pubkey: &str) -> Result<Option<InvoiceInfo>> {
        let mut conn = self.write_pool.get()?;

        let pubkey = pubkey.to_string();
        let pubkey_for_result = pubkey.clone();
...
LIMIT 1;
        "#;
            ...
            stmt.query_row(params![&pubkey], |r| {
...
/// Escape FTS5 MATCH special characters to prevent syntax errors.
/// FTS5: " - ( ) and similar. Wrap in quotes if contains special chars.
fn escape_fts5_term(s: &str) -> String {
    let needs_quoting = s.chars().any(|c| match c {
        '"' | '(' | ')' | '*' | '-' | '^' => true,
        _ => false,
    });
    if needs_quoting {
...
fn query_from_filter(f: &ReqFilter) -> (String, Vec<Box<dyn ToSql>>, Option<String>) {
    ...
    // NIP-50: when search is present, use FTS5. Index override not applicable with FTS JOIN.
    let use_fts = f.search.is_some();
    let idx_name = if use_fts {
        None
    } else {
        override_index(f)
    };
    ...
    let (mut query, mut params) = if let Some(ref search_term) = f.search {
        let term = escape_fts5_term(search_term.trim());
        let fts_query = "SELECT e.content FROM event e INNER JOIN event_fts ON e.id = event_fts.rowid WHERE event_fts MATCH ?";
        (fts_query.to_string(), vec![Box::new(term) as Box<dyn ToSql>])
    } else {
        (format!("SELECT e.content FROM event e {idx_stmt}"), vec![])
    };
    ...
    let where_or_and = if use_fts { " AND " } else { " WHERE " };
    query.push_str(where_or_and);
    query.push_str("hidden!=TRUE");
...
mod tests {
    ...
    fn test_query_nip50_search() {
        let filter = ReqFilter {
            ...
            authors: None,
            limit: Some(100),
            search: Some("hello world".to_owned()),
            tags: None,
            force_no_match: false,
        };

        let (query, params, _idx) = query_from_filter(&filter);
        assert!(query.contains("event_fts"));
        assert!(query.contains("MATCH ?"));
        assert!(query.contains("hidden!=TRUE"));
        assert_eq!(params.len(), 2); // search term + unix_time
    }
}
