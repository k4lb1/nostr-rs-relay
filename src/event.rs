...
/// Convert network event to parsed event.
/// NIP-01 validation (canonical, id, signature) is done by Nip01Handler in the relay pipeline.
impl From<EventCmd> for Result<EventWrapper> {
    fn from(ec: EventCmd) -> Result<EventWrapper> {
        ...
        if ec.cmd == "EVENT" {
            let mut e = ec.event;
            e.build_index();
            // update_delegation done by Nip26Handler in pipeline
            Ok(WrappedEvent(e))
        } else if ec.cmd == "AUTH" {
...
impl From<nostr::Event> for Event {
    fn from(nostr_event: nostr::Event) -> Self {
        Event {
            ...
            kind: nostr_event.kind.as_u16() as u64,
            tags: nostr_event.tags.iter().map(|x| x.clone().to_vec()).collect(),
