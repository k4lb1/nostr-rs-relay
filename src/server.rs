...
use crate::event::EventWrapper::{self, WrappedAuth, WrappedEvent};
use crate::info::RelayInfo;
use crate::nip05;
use crate::nip::grpc::GrpcHandler;
use crate::nip::kind_filter::KindFilterHandler;
use crate::nip::nip01::Nip01Handler;
use crate::nip::nip05_handler::Nip05Handler;
use crate::nip::nip09::Nip09Handler;
use crate::nip::nip26::Nip26Handler;
use crate::nip::nip42::Nip42Handler;
use crate::nip::nip50::Nip50Handler;
use crate::nip::pay_to_relay::PayToRelayHandler;
use crate::nip::whitelist::WhitelistHandler;
use crate::nip::NipContext;
...
use crate::error::Error::CommandUnknownError;
...
async fn handle_web_request(
    ...
    grpc: Option<Arc<tokio::sync::Mutex<crate::nauthz::EventAuthzService>>>,
    shutdown: Receiver<()>,
    ...
    const pubkeyInput = document.getElementById('pubkey-input');
      ...
      getPublicKeyBtn.addEventListener('click', async function() {
        ...
        ("/invoice", false) => {
            ...
            let payment_message;
            if let Ok((admission_status, _)) = repo.get_account_balance(&pubkey).await {
                if admission_status {
                    return Ok(Response::builder()
                        .status(StatusCode::OK)
...
pub fn start_server(settings: &Settings, shutdown_rx: MpscReceiver<()>) -> Result<(), Error> {
    ...
    rt.block_on(async {
        ...
        // build a repository for events (needed before admin_error_ctx for DM persistence)
        let repo = db::build_repo(&settings, metrics.clone()).await;

        // GRPC client for event admission (passed to handlers via NipContext)
        let grpc = if let Some(ref svr) = settings.grpc.event_admission_server {
            Some(Arc::new(tokio::sync::Mutex::new(
                crate::nauthz::EventAuthzService::connect(svr).await,
            )))
        } else {
            None
        };

        // Relay with NIP handler pipeline
        let mut relay_instance = relay::Relay::new(
            repo.clone(),
            settings.clone(),
            bcast_tx.clone(),
        );
        // Handler order: NIP-01 → KindFilter → NIP-26 → Whitelist → NIP-42 → NIP-05 → PayToRelay → GRPC → NIP-09 → NIP-50
        relay_instance.register_handler(Arc::new(Nip01Handler::new()));
        relay_instance.register_handler(Arc::new(KindFilterHandler::new()));
        relay_instance.register_handler(Arc::new(Nip26Handler::new()));
        relay_instance.register_handler(Arc::new(WhitelistHandler::new()));
        relay_instance.register_handler(Arc::new(Nip42Handler::new()));
        relay_instance.register_handler(Arc::new(Nip05Handler::new()));
        relay_instance.register_handler(Arc::new(PayToRelayHandler::new()));
        relay_instance.register_handler(Arc::new(GrpcHandler::new()));
        relay_instance.register_handler(Arc::new(Nip09Handler::new()));
        relay_instance.register_handler(Arc::new(Nip50Handler::new()));
        let relay = Arc::new(relay_instance);
        ...
        let admin_error_ctx = settings
            ...
            .and_then(|(sk, pk)| {
                let dm_repo = if settings.admin_error.send_dm {
                    Some(repo.clone())
                } else {
                    None
                };
                admin_error::AdminErrorContext::new(
                    sk,
                    pk,
                    settings.admin_error.rate_limit_secs,
                    settings.admin_error.send_dm,
                    dm_repo,
                )
                .map(Arc::new)
            });
        ...
        let make_svc = make_service_fn(|conn: &AddrStream| {
            ...
            let admin_error_ctx = admin_error_ctx.clone();
            async move {
                    ...
                    Ok(NostrMessage::EventMsg(ec)) => {
                        ...
                        match parsed {
                            Ok(WrappedEvent(e)) => {
                                ...
                                // NIP handler pipeline (NIP-01 validation, etc.)
                                let auth_pubkey_vec = conn.auth_pubkey().and_then(|p| hex::decode(p).ok());
                                let ctx = NipContext {
                                    auth_pubkey: auth_pubkey_vec.as_ref().map(|v| v.as_slice()),
                                    source_ip: conn.ip(),
                                    settings: &settings,
                                    repo: Some(&repo),
                                    origin: client_info.origin.as_deref(),
                                    user_agent: client_info.user_agent.as_deref(),
                                    payment_tx: Some(&payment_tx),
                                    grpc: grpc.as_ref(),
                                };
                                let processed = relay.process_event(e, ctx).await;
                                let (e, user_balance) = match processed {
                                    Ok(result) => result,
                                    Err(reason) => {
                                        let notice = Notice::blocked(evid.clone(), &reason);
                                ...
                                if e.is_expired() {
                                    let notice = Notice::invalid(e_id, "The event has already expired");
                                    if ws_stream.send(make_notice_message(&notice)).await.is_err() {
                                        debug!("failed to send notice, closing connection (cid: {})", cid);
                                        metrics.disconnects.with_label_values(&["send_error"]).inc();
                                        break;
                                    ...
                                    if let Some(fut_sec) = settings.options.reject_future_seconds {
                                        ...
                                        let notice = Notice::invalid(e_id.clone(), &msg);
                                        if ws_stream.send(make_notice_message(&notice)).await.is_err() {
                                            debug!("failed to send notice, closing connection (cid: {})", cid);
                                            metrics.disconnects.with_label_values(&["send_error"]).inc();
                                            break;
