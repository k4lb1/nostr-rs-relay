...
use nostr::{Event as NostrEvent, EventBuilder};
...
pub trait PaymentProcessor: Send + Sync {
    ...
    async fn get_invoice(&self, pubkey: &str, amount: u64) -> Result<InvoiceInfo, Error>;
...
impl Payment {
            ...
            m = self.payment_rx.recv() => {
                match m {
                    ...
                    Ok(PaymentMessage::CheckAccount(pubkey)) => {
                        if let Ok(Some(invoice_info)) = self.repo.get_unpaid_invoice(&pubkey).await
                        {
                            match self.check_invoice_status(&invoice_info.payment_hash).await? {
                                InvoiceStatus::Paid => {
                                    self.repo
                                        .admit_account(&pubkey, self.settings.pay_to_relay.admission_cost)
                                        .await?;
                                    self.payment_tx
                                        .send(PaymentMessage::AccountAdmitted(pubkey))
                                        .ok();
                                }
                                _ => {
                                    self.payment_tx
                                        .send(PaymentMessage::Invoice(pubkey, invoice_info))
                                        .ok();
                                }
                            }
                        } else {
                            let amount = self.settings.pay_to_relay.admission_cost;
                            let invoice_info =
                                self.get_invoice_info(&pubkey, amount).await?;
                            self.payment_tx
                                .send(PaymentMessage::Invoice(pubkey, invoice_info))
                                .ok();
                        }
                    ...
                    Ok(PaymentMessage::InvoicePaid(payment_hash)) => {
                        if self.check_invoice_status(&payment_hash).await?.eq(&InvoiceStatus::Paid) {
                            ...
                            self.repo.admit_account(&pubkey, self.settings.pay_to_relay.admission_cost).await?;
                        }
                    }
                    Ok(_) => {
    ...
    pub async fn send_admission_message(
        ...
        let receiver_pk = PublicKey::parse(pubkey)?;

        // Event DM with terms of service (NIP-04 encrypted)
        let terms_enc = nip04::encrypt(
            nostr_keys.secret_key(),
            &receiver_pk,
            &self.settings.pay_to_relay.terms_message,
        )?;
        let message_event: NostrEvent = EventBuilder::new(
            nostr::event::Kind::EncryptedDirectMessage,
            terms_enc,
        )
        .tag(nostr::Tag::public_key(receiver_pk))
        .sign_with_keys(nostr_keys)?;

        // Event DM with invoice
        let invoice_enc = nip04::encrypt(
            nostr_keys.secret_key(),
            &receiver_pk,
            &invoice_info.bolt11,
        )?;
        let invoice_event: NostrEvent = EventBuilder::new(
            nostr::event::Kind::EncryptedDirectMessage,
            invoice_enc,
        )
        .tag(nostr::Tag::public_key(receiver_pk))
    ...
    pub async fn get_invoice_info(&self, pubkey: &str, amount: u64) -> Result<InvoiceInfo> {
        ...
        let invoice_info = self.processor.get_invoice(pubkey, amount).await?;

        // Persist invoice to DB
        self.repo
            .create_invoice_record(pubkey, invoice_info.clone())
            .await?;
